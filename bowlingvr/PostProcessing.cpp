...
GeneralFramebuffer::GeneralFramebuffer(float w, float h)
{
	glGenFramebuffers(1, &fbo);
	...
	glBindTexture(GL_TEXTURE_2D, fbo_texture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, w, h, 0, GL_BGR, GL_UNSIGNED_INT, NULL);
...
void GeneralFramebuffer::BindFBO()
{
	glBindFramebuffer(GL_FRAMEBUFFER, fbo);
}

void GeneralFramebuffer::UnbindFBO()
{
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
}

void GeneralFramebuffer::Bind()
{
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, fbo_texture);
}
